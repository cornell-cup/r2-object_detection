...
def replace_with_rrt(path_hd, path_tl, obstacles):
    ...
    g = rrt.rrt(rrt_start.angles, rrt_end.angles, obstacles)
    if g.success:
        print("rrt success :)")
    else:
        print("rrt failed :(")
...
def linear_rrt_test(num_trials, obstacles, iter_per_path=10):
    ...
    for i in range(num_trials):
        ...
        end_pos = random_angle_config()
        end_node = RRTNode(end_pos)

        while (not arm.angles_within_bounds(start_pos) and valid_path_configuration(start_node, obstacles)) \
                or (not arm.angles_within_bounds(end_pos) and valid_path_configuration(end_node, obstacles)):
            start_pos = random_angle_config()
            end_pos = random_angle_config()

        _, success = linear_rrt(start_pos, end_pos, iter_per_path, obstacles)
...
def plot_random_path(iterations, obstacles, arm):
    ...
    while not (arm.angles_within_bounds(start_pos) and arm.angles_within_bounds(end_pos)):
        start_pos = random_angle_config()
        end_pos = random_angle_config()

    start_pos = [0, 3 * math.pi / 2, math.pi / 2, 0, 0]

    path, _ = linear_rrt(start_pos, end_pos, iterations, obstacles)

    g = Graph(start_pos, end_pos)
