...
import time
import random
from collision_detection import arm_is_colliding_prisms, newLineCollider
...
def rrt_graph_list(num_trials, n_iter, radius, step_size, threshold, bounds, num_obstacles=1):
    ...
    return graphs, failed_obstacles, paths
...
def path_optimizer_four(path, prism):
    ...
    for i in range(0, len(path)-4, 4):
        ...
        #print(newLineCollider(line_seg, prism))
        if newLineCollider(line_seg, prism) == None:
...
if __name__ == '__main__':
    ...
    num_trials = 100
    graphs, _, paths = rrt_graph_list(num_trials, n_iter, radius, stepSize, threshold, bounds, num_obstacles=3)

    print("avg dist", tpm.avg_distance_traveled_test(paths))
